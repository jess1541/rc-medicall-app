...
const ExecutiveCalendar: React.FC<ExecutiveCalendarProps> = ({ doctors, onUpdateDoctors, onDeleteVisit, user }) => {
  ...
  const saveReport = () => {
      ...
      if (!reportNote.trim() || !reportFollowUp.trim()) {
          alert("Detalles de Visita y Objetivo Próximo son obligatorios.");
          return;
      }
      ...
      const updatedDoctors = doctors.map(doc => {
          if (doc.id === selectedVisitToReport.docId) {
              ...
              if (nextVisitDate) {
                  const nextGoal = reportFollowUp.toUpperCase();
                  const newVisit: Visit = {
                      id: `nv-${Date.now()}`,
                      date: formatDateToString(nextVisitDate),
                      time: nextVisitTime, 
                      note: nextGoal, // Se coloca el objetivo como resultado planeado inicial
                      objective: nextGoal, 
                      outcome: 'PLANEADA',
                      status: 'planned'
                  };
      ...
      onUpdateDoctors(updatedDoctors);
      setReportModalOpen(false);
      if (nextVisitDate) alert("Reporte guardado y próxima visita agendada correctamente.");
  };
  ...
  return (
    <div className="space-y-6 pb-10 relative">
       ...
       {/* 2. Report/Edit Modal - Updated with new fields and scheduling logic */}
           ...
           <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
               <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                   ...
                   <div className="p-6 space-y-5 overflow-y-auto">
                       ...
                       {isEditingPlan ? (
                           <div>
                               <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Objetivo de la Visita (Editar)</label>
                               <textarea 
                                   rows={3}
                                   value={editObjective}
                           ...
                           <>
                               ...
                               <div>
                                   <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Resultado Comercial</label>
                               ...
                               <div>
                                   <label className="block text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2">
                                       <Edit3 className="w-3 h-3 text-blue-500" /> Detalles de la Visita
                                   </label>
                                   <textarea 
                                       rows={3}
                                       value={reportNote}
                                       ...
                                       placeholder="DESCRIBA QUÉ SUCEDIÓ DURANTE LA VISITA..."
                                   />
                               </div>

                               <div>
                                   <label className="block text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2">
                                       <CalendarClock className="w-3 h-3 text-indigo-500" /> Objetivo de la Siguiente Visita
                                   </label>
                                   <textarea 
                                       rows={2}
                                       ...
                                       className="w-full border border-slate-200 rounded-xl p-3 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none uppercase resize-none"
                                       placeholder="¿CUÁL ES EL SIGUIENTE PASO CON ESTE CONTACTO?..."
                                   />
                               </div>
                               ...
                               <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 mt-2">
                                   <div className="flex items-center mb-3">
                                       <input 
                                            ...
                                            className="w-5 h-5 text-blue-600 rounded-lg focus:ring-blue-500 mr-3 cursor-pointer shadow-sm"
                                       />
                                       <label htmlFor="scheduleNext" className="text-sm font-black text-blue-900 uppercase tracking-tight cursor-pointer">Agendar Próxima Visita</label>
                                   </div>
                                   
                                   {nextVisitDate && (
                                       <div className="grid grid-cols-2 gap-4 mt-2 animate-fadeIn">
                                           <div>
                                               <label className="block text-[10px] font-black text-blue-500 uppercase mb-1 ml-1">Seleccionar Fecha</label>
                                               <DatePicker 
                                                    selected={nextVisitDate} 
                                                    onChange={(date) => setNextVisitDate(date)} 
                                                    dateFormat="dd/MM/yyyy"
                                                    locale="es"
                                                    className="w-full text-xs font-bold p-3 rounded-xl border border-blue-200 bg-white focus:ring-2 focus:ring-blue-500 outline-none shadow-sm"
                                               />
                                           </div>
                                           <div>
                                               <label className="block text-[10px] font-black text-blue-500 uppercase mb-1 ml-1">Seleccionar Hora</label>
                                               <select 
                                                    value={nextVisitTime} 
                   ...
                   <div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                       ...
                       {isEditingPlan ? (
                           ...
                           <button onClick={saveReport} className="px-6 py-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-green-500/30 transition-all active:scale-95 flex items-center">
                               <CheckCircle2 className="w-4 h-4 mr-2" /> Finalizar Reporte
                           </button>
                       )}
                   </div>
               </div>
