...
const OperationsDashboard: React.FC<OperationsDashboardProps> = ({ operations, procedures, doctors }) => {
  ...
  return (
    <div className="space-y-6 pb-16 animate-fadeIn">
      {/* HERO SECTION */}
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6 bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-slate-300 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center gap-5 md:gap-8 text-center md:text-left z-10">
            ...
            <div>
                <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter leading-tight">
                    Dashboard <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-700 to-purple-600">Operativos</span>
                </h1>
                <p className="text-slate-500 font-black uppercase tracking-[0.25em] text-[10px] mt-2 flex items-center gap-2 justify-center md:justify-start">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto justify-center z-10">
             <button 
                ...
                className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-emerald-500/20 transition-all flex items-center gap-2 active:scale-95 border border-emerald-500/20"
             >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-md border border-slate-200 relative overflow-hidden group hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
          ...
          <Stethoscope className="absolute -right-4 -bottom-4 w-24 h-24 text-slate-100 group-hover:text-indigo-500/5 transition-colors duration-300" />
          
          <div className="relative z-10">
              <div className="w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center mb-4 group-hover:bg-indigo-100 transition-colors">
                  <Stethoscope className="w-5 h-5 text-slate-500 group-hover:text-indigo-600" />
              </div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Procedimientos (Mes)</p>
              <div className="flex items-baseline gap-2">
                  <span className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">{stats.totalProcedures}</span>
                  <span className="text-[10px] font-bold text-indigo-600 uppercase">Registrados</span>
              </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-md border border-slate-200 relative overflow-hidden group hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="relative z-10">
              ...
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Venta Mensual</p>
        ...
        {/* Card 3: Comisiones */}
        <div className="bg-gradient-to-br from-purple-600 to-fuchsia-700 p-6 md:p-8 rounded-[2.5rem] shadow-xl shadow-purple-500/20 text-white relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300 border border-white/10">
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {stats.technicianStats.map((tech, idx) => (
                  <div key={idx} className="bg-white p-6 rounded-[2rem] shadow-md border border-slate-200 hover:shadow-xl transition-all group hover:-translate-y-1">
                      <div className="flex justify-between items-start mb-4">
                          <div className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-colors">
                  ...
                  <tbody className="divide-y divide-slate-50">
                      {recentProcedures.map((op, i) => (
                          <tr key={i} className="group hover:bg-slate-50 transition-colors">
                              ...
                              <td className="py-5">
                                  <span className="text-xs font-medium text-slate-600 uppercase">{op.remissionNumber || '-'}</span>
                              </td>
                              <td className="py-5">
                                  <span className="text-xs font-medium text-slate-600 uppercase">{op.hospital || '-'}</span>
                              </td>
                              <td className="py-5">
                                  <span className="text-xs font-medium text-slate-600 uppercase">{op.type}</span>
                              </td>
                              <td className="py-5">
                                  <span className="text-xs font-medium text-slate-600 uppercase">{op.technician || '-'}</span>
                              </td>
                              <td className="py-5">
                                  <span className="text-xs font-medium text-slate-600 uppercase">{op.executive || '-'}</span>
                              </td>
                              <td className="py-5">
